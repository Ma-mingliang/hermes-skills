...
3. Is there a skill for this? → Check local skills (`skills_list`) AND search GitHub for high-star alternatives
4. Is there a GitHub implementation/template? → Run GitHub code search for maintained OSS before writing net-new code

### Skill-Specific Search (Hermes Agent)

When the user asks to create a new skill, ALWAYS search first:

1. **Local skills**: `skills_list` → check if any existing skill covers the territory
2. **GitHub search**: Use GitHub API to find high-star alternatives:
   ```python
   import requests
   url = "https://api.github.com/search/repositories"
   params = {"q": "hermes skill <topic>", "sort": "stars", "order": "desc", "per_page": 10}
   headers = {"Accept": "application/vnd.github.v3+json", "User-Agent": "Hermes-Agent"}
   response = requests.get(url, params=params, headers=headers, timeout=10)
   ```
3. **Compare from principles**: Don't just list features — analyze what fundamentally differs between approaches
4. **Only create if gap is real**: If an existing solution covers 80%+ of the need, use/extend it

**User preference**: "先搜索已有方案再创建" and "对比其他方案从原理角度拆分"
